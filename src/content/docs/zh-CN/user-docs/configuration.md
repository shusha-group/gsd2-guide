...
| `GSD_ALLOWED_COMMAND_PREFIXES` | （内置列表） | 允许用于 `!command` 值解析的命令前缀，逗号分隔。会覆盖 settings.json 中的 `allowedCommandPrefixes`。见 [自定义模型：命令允许列表](../custom-models/#command-allowlist)。 |
...
- `execution_simple`：用于被 [complexity router](../token-optimization/#complexity-based-task-routing) 判断为 “simple” 的 task
...
关于 provider 配置、model overrides、OpenAI compatibility 和更多高级示例，见 [自定义模型指南](../custom-models/)。
...
负责协调 model 选择、阶段跳过和上下文压缩。详见 [Token 优化](../token-optimization/)。
...
原因：`osascript display notification` 的通知权限是算在你的终端应用（Ghostty、iTerm2 等）上的，而这些应用在 System Settings → Notifications 中未必被允许。`terminal-notifier` 会注册成独立 App，并在首次使用时主动请求通知权限。如果通知异常，见 [故障排查：macOS 上通知不显示](../troubleshooting/#notifications-not-appearing-on-macos)。
...
基于复杂度的 model 路由。详见 [动态模型路由](../dynamic-model-routing/)。
...
完整细节见 [并行编排](../parallel-orchestration/)。
