...
MCP server 会暴露 GSD 的完整 workflow 工具面：milestone planning、task completion、slice 管理、roadmap reassessment、journal 查询等。会话管理工具（`gsd_execute`、`gsd_status`、`gsd_result`、`gsd_cancel`）允许 Claude Code 启动并监控 GSD 自动模式会话。完整工具列表见 [命令 → MCP Server 模式](../commands/#mcp-server-mode)。
...
注意：这里的 `apiKey` 字段写的是**环境变量名**，不是字面 key。GSD 会自动解析它。你也可以改用字面值或 shell 命令（见 [值解析](../custom-models/#value-resolution)）。
...
关于 `compat` 字段、`modelOverrides`、值解析和高级配置的完整说明，见 [自定义模型](../custom-models/)。
...
如果还需要更多帮助，请查看 [故障排查](../troubleshooting/)，或者在会话中运行 `/gsd doctor`。
