...
如果启用了 [并行编排](../parallel-orchestration/)，多个 milestones 可以同时运行在各自独立的 worktree 中：
...
这样会把 milestone 分支推送到远程，并创建一个目标分支为 `develop`（或你指定的其它分支）的 PR。要求已安装并认证 `gh` CLI。详见 [git.auto_pr](../configuration/#gitauto_pr)。
