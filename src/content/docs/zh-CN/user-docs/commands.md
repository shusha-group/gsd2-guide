...
完整文档见 [并行编排](../parallel-orchestration/)。
...
| `gsd config` | 配置搜索和文档工具所需的全局 API keys（保存到 `~/.gsd/agent/auth.json`，对所有项目生效）。见 [Global API Keys](../configuration/#global-api-keys-gsd-config)。 |
| `gsd update` | 更新到最新版本 |
