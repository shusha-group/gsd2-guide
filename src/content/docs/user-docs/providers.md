...
Note: the `apiKey` field here is the *name* of the environment variable, not the literal key. GSD resolves it automatically. You can also use a literal value or a shell command (see [Value Resolution](../custom-models/#value-resolution)).
...
For the full reference on `compat` fields, `modelOverrides`, value resolution, and advanced configuration, see [Custom Models](../custom-models/).
...
For additional help, see [Troubleshooting](../troubleshooting/) or run `/gsd doctor` inside a session.
