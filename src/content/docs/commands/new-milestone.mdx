...
The command accepts an optional milestone name as an argument. If no `MILESTONE-CONTEXT.md` file exists (a pre-gathered context file you can prepare in advance), it falls back to interactive Q&A — asking what you want to build next and exploring features, priorities, and constraints.
...
flowchart TD
    ...
    ContextFile -->|No| QA[Freeform ask: What to build next?\nthen AskUserQuestion for specifics]
...
**2. Gather milestone goals** — If `MILESTONE-CONTEXT.md` exists (a pre-prepared context file), its features and scope are used and presented for confirmation. Otherwise, GSD shows what shipped in the last milestone and asks inline (freeform): "What do you want to build next?" — then follows up with `AskUserQuestion` to probe features, priorities, constraints, and scope.
...
**6. Resolve models** — Runs `gsd-tools init new-milestone` to extract `researcher_model`, `synthesizer_model`, `roadmapper_model`, and workflow config flags.

**7. Research decision** — Asks whether to research the domain ecosystem for new features before defining requirements. The choice is persisted to config so future commands honor it. If "Research first" is selected, 4 parallel `gsd-project-researcher` agents run (see below).

**8. Research (optional)** — Spawns 4 parallel agents, each focused on a different dimension:
...
**11. Final commit** — Commits `ROADMAP.md`, `STATE.md`, and `REQUIREMENTS.md` together (with the traceability section filled in by the roadmapper). Displays a completion summary with the artifact table and the next command.
...
If a `MILESTONE-CONTEXT.md` file is present in `.planning/` when you run new-milestone, the Q&A step is skipped entirely — GSD uses that file's features and scope directly and presents a summary for confirmation. This is the fastest path through new-milestone when the goals are already clear.
...
- [`/gsd discuss-phase`](../discuss-phase/) — Clarify implementation decisions for a specific phase before planning
