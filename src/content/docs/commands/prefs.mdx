...
flowchart TD
    ...
    Cat -->|Skills| CfgSkills[discovery, staleness days, UAT dispatch]
...
| **Skills** | Skill discovery mode, staleness threshold, and UAT dispatch behavior | `skill_discovery` (`auto`/`suggest`/`off`), `skill_staleness_days`, `uat_dispatch` |
...
- **`skill_staleness_days`** — Skills that haven't been used in this many days are deprioritized during discovery. Set to `0` to disable staleness tracking. Defaults to `60`.
