...
flowchart TD
    ...
    Cat -->|Git| CfgGit[branch, push, isolation, merge strategy]
...
| **Models** | Model selection per execution phase | `models.research`, `models.planning`, `models.execution`, `models.completion` |
| **Timeouts** | Auto-mode supervisor timeouts | `auto_supervisor.soft_timeout_minutes`, `idle_timeout_minutes`, `hard_timeout_minutes` |
| **Git** | Branch naming, push behavior, isolation, merge strategy | `git.main_branch`, `git.auto_push`, `git.push_branches`, `git.snapshots`, `git.remote`, `git.pre_merge_check`, `git.commit_type`, `git.merge_strategy`, `git.isolation` |
| **Skills** | Skill discovery and UAT dispatch behavior | `skill_discovery` (`auto`/`suggest`/`off`), `uat_dispatch` |
...
Selecting `solo` or `team` sets a bundle of git defaults as the lowest-priority layer. Explicit values you've set always win.
...
❯ Budget
  ...
  Budget enforcement (current: warn):
  ❯ warn
    pause
    halt
