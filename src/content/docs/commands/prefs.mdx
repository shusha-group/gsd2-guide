...
flowchart TD
    ...
    Cat -->|Git| CfgGit[branch, push, snapshots, commit type, isolation, merge strategy, stale threshold]
...
| **Models** | Model selection per execution phase, with optional per-phase fallbacks | `models.research`, `models.planning`, `models.discuss`, `models.execution`, `models.execution_simple`, `models.completion`, `models.validation`, `models.subagent` |
| **Timeouts** | Auto-mode supervisor timeouts | `auto_supervisor.soft_timeout_minutes`, `idle_timeout_minutes`, `hard_timeout_minutes` |
| **Git** | Branch naming, push behavior, WIP snapshots, commit type, isolation, merge strategy, stale threshold | `git.main_branch`, `git.auto_push`, `git.push_branches`, `git.snapshots`, `git.remote`, `git.pre_merge_check`, `git.commit_type`, `git.merge_strategy`, `git.isolation`, `git.absorb_snapshot_commits`, `stale_commit_threshold_minutes` |
...
The model wizard groups available models by provider. For each of the eight phases (`research`, `planning`, `discuss`, `execution`, `execution_simple`, `completion`, `validation`, `subagent`) you pick a provider first, then select a specific model ID from that provider's list. You can also type a model ID manually or clear the phase back to the inherited default.
...
| `isolation` | `none` | `worktree`, `branch`, or `none` |
| `absorb_snapshot_commits` | `true` | Absorb WIP snapshot commits into real commits when committing |
| `stale_commit_threshold_minutes` | `30` | Minutes without a commit before flagging dirty working tree as stale; `0` disables |

`pre_merge_check: "auto"` lets GSD detect whether CI checks are configured and run them only when present.

`stale_commit_threshold_minutes` is a top-level preference (not nested under `git`) but is displayed and configured inside the Git wizard category for convenience. When the threshold is exceeded and the working tree is dirty, GSD's doctor will auto-commit a safety snapshot tagged with `[gsd safety]`.
...
- **`uat_dispatch`** — When `true`, GSD dispatches a UAT agent after each slice completes. Default: `false`.
