...
**Ghost milestones** (directories containing only `META.json` with no `CONTEXT`, `ROADMAP`, or `SUMMARY` file) are silently skipped — they don't appear in the registry.
...
| `blocked` | All remaining milestones have unmet dependencies |
...
- **Milestone progress** — Active milestone ID and title, followed by filled progress bars for Tasks (active slice), Slices (current milestone), and Milestones (overall). Parked milestones are shown distinctly.
