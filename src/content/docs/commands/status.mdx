...
`/gsd status` opens a TUI dashboard overlay that shows the current state of your project at a glance — what auto-mode is doing right now, milestone and slice progress, task breakdown for the active slice, cost metrics, and environment health. The dashboard refreshes every 2 seconds.

The dashboard is read-only. It doesn't modify any files or interrupt a running auto-mode session. You can open it while auto-mode is running to monitor progress without disrupting execution. When no session is active, it still shows project state (phase, milestone, slice, tasks remaining) derived from the planning database and files on disk.
...
The dashboard opens the planning database, derives project state, then overlays real-time data from the running auto-mode session.

```mermaid
flowchart TD
    Start(["/gsd status or Ctrl+Alt+G"]) --> EnsureDb[ensureDbOpen — open gsd.db]
    EnsureDb --> DeriveState[deriveState — DB-primary, filesystem fallback]
    DeriveState --> CheckMilestones{Milestones found?}
    ...
    FetchLive --> LoadMetrics[Load metrics ledger]
    ...
    style EnsureDb fill:#1a3a1a,stroke:#39ff14,color:#e8f4e8
...
1. **Database (primary)** — `deriveState()` queries the `gsd.db` SQLite database for milestone, slice, and task state. Before calling `deriveState()`, the command explicitly opens the database so cold sessions use DB-backed state rather than the filesystem fallback. Results are memoized for 100ms so repeated calls within a single refresh cycle don't hit the database multiple times.
2. **Planning files (fallback)** — When the database is unavailable or empty (e.g. unmigrated projects or first run), `deriveState()` reads roadmap and plan files directly from disk to determine current phase, active milestone, active slice, and task completion state. When the native Rust batch parser is available, all `.md` files under `.gsd/` are read in a single pass and served from an in-memory cache.

For live session data, `getAutoDashboardData()` returns in-memory state from the running auto-mode session: what unit is executing right now, how long it's been running, pending capture count, and whether the session is active, paused, or idle. When another auto-mode session is detected via `auto.lock`, the dashboard shows that remote session's PID and current unit instead.
...
`deriveState()` builds a registry of every milestone and assigns each one a status:
...
The dashboard header reflects the current phase derived from disk:
...
| `evaluating-gates` | Quality gates are being evaluated in parallel during slice planning |
...
- **Header** — "GSD Dashboard" title, blinking `● AUTO` / `⏸ PAUSED` / `idle` status indicator, elapsed time, and ETA if estimable. Shows the active worktree name when running inside a worktree. When a remote auto-mode session is detected, shows the remote session's PID and started-at time instead of the local elapsed timer.
- **Progress score** — Visible when auto-mode is active or paused. A traffic-light `●` (green / yellow / red) with a one-line summary. When degraded, individual signal lines are shown below the summary.
- **Current unit** — What's actively running (unit type and ID), how long it's been executing. Shows "No unit running · /gsd auto to start" when idle, "/gsd auto to resume" when paused, or the remote session's current unit type and ID when another session is active.
...
- **Cost & Usage** — Total cost (or request count for subscription users), tokens, tool calls, and unit count. Token breakdown by input / output / cache-read / cache-write. Budget aggregate line (truncation sections and continue-here count) when applicable. Cost broken down by phase and by slice. Per-slice cost projection for remaining work (if budget ceiling is configured). Cost broken down by model with context window size. Average cost and token count per unit. Cache hit rate when non-zero. RTK token savings when RTK is enabled and has accumulated data.
...
The dashboard polls every 2 seconds. If auto-mode completes a unit while the dashboard is open, you'll see the milestone and task progress update within a couple of seconds. The refresh is skipped if a previous refresh is still in flight, preventing pile-up on slow disks. Terminal resize events also trigger an immediate re-render.
...
| File | Purpose |
|------|---------|
| `.gsd/gsd.db` | Primary source for milestone, slice, and task state (SQLite database) |
...
| `M*/<MID>-ROADMAP.md` | Slice structure, ordering, and completion state (filesystem fallback) |
...
| `M*/S*/<SID>-PLAN.md` | Task breakdown within the active slice (filesystem fallback) |
