...
flowchart TD
    ...
    StopLocal --> Teardown
    Remote --> Teardown
    Teardown[Clear auto.lock + OS lock + crash lock] --> Skills[Clear skill state]
    ...
    Rebuild --> Cleanup[Reset metrics, hooks,\nmodel, browser]
...
2. **Clear session lock** — Removes `.gsd/auto.lock`, releases the OS-level lock held via `proper-lockfile` (the `.gsd.lock/` directory), and clears the crash lock.
...
10. **Reset session state** — Clears metrics, routing history, hook state, persisted hook state, browser processes, and in-flight tool records. Restores the original model if it was changed during the session. Removes `.gsd/runtime/paused-session.json` if it exists.
