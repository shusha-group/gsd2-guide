...
flowchart TD
    ...
    Validate -->|Yes| Empty{Description empty?}
    Empty -->|Yes| Usage([Show usage])
    Empty -->|No| Slug[Generate slug from description]
    Slug --> Num[Determine next task number]
    Num --> Dir[Create task dir .gsd/quick/N-slug/]
    Dir --> IsoCheck{isolation != none?}
    IsoCheck -->|No| Notify[Notify user]
    IsoCheck -->|Yes| BranchCreate[Auto-commit dirty state]
    BranchCreate --> Branch[Create branch gsd/quick/N-slug]
    Branch -->|Success| Persist[Persist quick-return.json]
    Branch -->|Failure| WarnBranch[Warn — continue on current branch]
    Persist --> Notify
    WarnBranch --> Notify
    Notify --> Dispatch[Dispatch task to agent]
    Dispatch --> Execute[Agent executes task]
    Execute --> Summary[Write N-SUMMARY.md]
    Summary --> CleanupCheck{quick-return.json exists?}
    CleanupCheck -->|No| Done([Complete])
    CleanupCheck -->|Yes| Squash[Squash-merge back to original branch]
    Squash --> Cleanup[Delete quick branch + clear runtime state]
    Cleanup --> Done
    ...
    style IsoCheck fill:#0d180d,stroke:#39ff14,color:#39ff14
    style CleanupCheck fill:#0d180d,stroke:#39ff14,color:#39ff14
    style Fail fill:#3a1a1a,stroke:#ff4444,color:#e8e8e8
    style Usage fill:#3a1a1a,stroke:#ff4444,color:#e8e8e8
    style WarnBranch fill:#3a1a1a,stroke:#ff6600,color:#e8e8e8
    ...
    style BranchCreate fill:#1a3a1a,stroke:#39ff14,color:#e8f4e8
    style Branch fill:#1a3a1a,stroke:#39ff14,color:#e8f4e8
    style Persist fill:#1a3a1a,stroke:#39ff14,color:#e8f4e8
    ...
    style Squash fill:#1a3a1a,stroke:#39ff14,color:#e8f4e8
...
- [`/gsd history`](../history/) — Review what quick tasks and units have been executed
