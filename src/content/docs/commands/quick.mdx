...
flowchart TD
    ...
    Branch --> Persist[Persist quick-return.json]
    Persist --> Dir[Create task dir .gsd/quick/N-slug/]
    ...
    StateUpdate --> Squash[Squash-merge back to original branch]
...
| `.gsd/runtime/quick-return.json` | Persisted return state (original branch, quick branch, task info) for cleanup across session resets |
| `gsd/quick/N-slug` branch | Git branch for the change (deleted after squash-merge) |
