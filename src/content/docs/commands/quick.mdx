...
4. **Create branch** — Creates a git branch named `gsd/quick/N-slug`. If your working tree has uncommitted changes, GSD auto-commits them first with a `chore(QN): auto-commit after quick-task` message before switching. If branch creation fails for any reason, GSD continues on the current branch with a warning.
