...
/gsd undo        /gsd undo-task     /gsd reset-slice   /gsd skip
/gsd export      /gsd cleanup       /gsd model         /gsd mode
/gsd steer       /gsd knowledge     /gsd new-milestone /gsd parallel
/gsd prefs       /gsd config        /gsd hooks         /gsd run-hook
/gsd skill-health /gsd doctor       /gsd forensics     /gsd migrate
/gsd remote      /gsd inspect       /gsd update        /gsd park
/gsd unpark      /gsd init          /gsd setup         /gsd keys
/gsd logs        /gsd cmux          /gsd start         /gsd templates
/gsd extensions  /gsd fast          /gsd changelog     /gsd rate
/gsd widget      /gsd next          /gsd workflow      /gsd rethink
/gsd codebase    /gsd notifications /gsd mcp
```
...
flowchart TD
    ...
    DirCheck -->|OK| HasGSD{.gsd/ bootstrapped?}
    WarnConfirm --> HasGSD
    ...
    InitWizard --> SelfHeal
    HasGSD -->|Yes| SelfHeal[Self-heal stale runtime records]
    SelfHeal --> Interrupted{Interrupted\nsession?}
    Interrupted -->|Running| ErrorExit([Error: session still running])
    Interrupted -->|Stale| DeriveState[Clear silently]
    Interrupted -->|Recoverable| RecoverPrompt[Recovery prompt]
    DeriveState --> DeriveState2[Derive state]
    RecoverPrompt -->|Resume| Auto(["/gsd auto or /gsd next"])
    RecoverPrompt -->|Continue manually| DeriveState2
    DeriveState2 --> SmartEntry[Smart entry wizard]
    SmartEntry --> Execute[Execute one unit]
    ...
    style Interrupted fill:#0d180d,stroke:#39ff14,color:#39ff14
    style HasGSD fill:#0d180d,stroke:#39ff14,color:#39ff14
    style Exit fill:#3a1a1a,stroke:#ff4444,color:#e8e8e8
    style ErrorExit fill:#3a1a1a,stroke:#ff4444,color:#e8e8e8
    ...
    style SelfHeal fill:#1a3a1a,stroke:#39ff14,color:#e8f4e8
    style DeriveState fill:#1a3a1a,stroke:#39ff14,color:#e8f4e8
    style RecoverPrompt fill:#1a3a1a,stroke:#39ff14,color:#e8f4e8
    style DeriveState2 fill:#1a3a1a,stroke:#39ff14,color:#e8f4e8
...
Staleness detection handles interrupted sessions: if the pending entry is older than 30 seconds and neither the discussion manifest nor the milestone's `CONTEXT.md` exist on disk, the entry is cleared and the flow restarts from scratch.
...
After bootstrapping, `/gsd` calls `assessInterruptedSession` to check for any previous session that didn't end cleanly. Three classifications are possible:

- **Running** — A session lock exists and the owning process is still alive. `/gsd` shows an error and exits to avoid interfering with the live session.
- **Stale** — A lock or paused-session file exists but the process is gone and the session is too old to recover. Cleared silently.
- **Recoverable** — A recent interrupted session (crash or pause) with recoverable state. `/gsd` presents a choice:
  - **Resume with /gsd next** (if the interrupted session was in step mode) or **Resume with /gsd auto** (if it was auto mode) — restart from where it left off.
...
If no `.gsd/` directory exists at all (or it's an incomplete "zombie" state with no `PREFERENCES.md` or `milestones/` directory), `/gsd` runs the project init wizard first. It detects project type, configures GSD, and bootstraps the `.gsd/` tree. It also ensures a git repository exists — initialising one if needed (or if the directory is inside an inherited repo from a parent directory). Once init completes, the normal flow continues — which will detect that no milestones exist and start the discuss prompt.

### Discussion preparation

Before dispatching a new milestone discussion, GSD runs an optional **preparation phase** that produces structured briefs to inject into the discuss prompt. This is enabled by default and can be disabled via `discuss_preparation: false` in preferences.

When preparation runs, it generates two briefs:
- **Codebase brief** — what already exists in the project, established patterns, and constraints
- **Prior context brief** — summaries of completed milestones so the discussion can build on prior work

Both briefs are injected into the standard `discuss` prompt as a `Preparation Context` section. The discuss template still leads with "What's the vision?" — the preparation context is background grounding, not a replacement for the user's direction. Ecosystem research now happens during the discussion session itself, between question rounds.

If preparation fails or is disabled, the discuss prompt is dispatched without the context section.
...
**No roadmap yet** — Context exists but no roadmap (corresponds to `pre-planning` state). Offers to create a roadmap (`guided-plan-milestone`) or discuss first (`guided-discuss-milestone`). Also allows skipping or discarding the milestone entirely.

**Roadmap exists, no active slice** — Offers to go auto, view status, or take milestone-level actions via a submenu (park, discard, or skip).
...
**Status fallback** — Phases that have no interactive wizard path — including `validating-milestone`, `completing-milestone`, `discussing`, `verifying`, `advancing`, `evaluating-gates`, `researching`, and `blocked` — fall through to the status view. Use [`/gsd auto`](../auto/) or [`/gsd dispatch`](../cli-flags/) to drive these phases forward.
...
● GSD — M002: Sharing & Social
  ...
  ❯ Discuss from draft (Recommended)
...
- [`discuss`](../../prompts/discuss/) — Interactive milestone planning prompt (vision capture, Q&A, requirements, roadmap). Preparation context is injected via `{{preparationContext}}` when the preparation phase runs.
...
- [`/gsd park`](../park/) — Park a milestone (skip without deleting, reactivate with `/gsd unpark`)
- [`/gsd queue`](../queue/) — Add future milestones to the pipeline
- [`/gsd rethink`](../rethink/) — Conversational project reorganization — reorder, park, discard, or add milestones
