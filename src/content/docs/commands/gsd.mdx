...
Under the hood, `/gsd` calls the same `startAuto()` function as [`/gsd auto`](../auto/), but with `step: true`. This means the full initialization sequence runs — stale worktree cleanup, crash recovery, state derivation, worktree setup — but after each unit completes, execution pauses and a step wizard appears.
...
This means you can go from zero to a running milestone by just typing `/gsd` and talking.
...
- A **summary** (compressing completed work for future context)
- A **roadmap reassessment** (re-evaluating remaining slices after a slice completes)

The dispatch engine [determines the unit type](../auto/#the-dispatch-loop) by reading the current project state from `.gsd/` files.
...
| `S*-PLAN.md` | Task breakdown for the active slice |
| `T*-SUMMARY.md` | Prior task outcomes |
...
| `T*-SUMMARY.md` | Written by task executors |
