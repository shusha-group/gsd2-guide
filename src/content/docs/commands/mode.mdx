...
Before opening the mode selector, `/gsd mode` calls `ensurePreferencesFile`. If the target preferences file doesn't exist, it creates one from the built-in template before proceeding. You won't be dropped into the selector against a missing file.
