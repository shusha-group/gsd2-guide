...
`/gsd doctor` scans your `.gsd/` directory for structural integrity issues — missing summaries, unchecked roadmap entries, stale locks, orphaned worktrees, corrupt git state, environment misconfigurations, missing API keys, database constraint violations, and more. It operates in four modes:
...
flowchart TD
    ...
    Scan[Run all check domains] --> Git[Git health<br/>Orphaned worktrees, stale branches,<br/>corrupt merge state, stale commits]
    ...
    Scan --> Engine[Engine health<br/>DB constraint violations,<br/>projection drift]
    ...
    Filter --> HasErrors{Errors remain?}
    ...
    style Engine fill:#1a3a1a,stroke:#39ff14,color:#e8f4e8
...
- **`GLOBAL_STATE_CODES`** — `orphaned_project_state` and `orphaned_completed_units`. Removing completed-unit keys can revert project state, treating finished tasks as incomplete. Removing orphaned project state directories deletes external state. These are only fixed by explicit manual doctor runs (`/gsd doctor fix`).

All other fixable codes are fixed immediately even at `fixLevel: "task"` — including roadmap checkbox updates (`all_tasks_done_roadmap_not_checked`) and UAT stubs (`all_tasks_done_missing_slice_uat`) — to keep bookkeeping consistent after each task.
...
| `numbered_gsd_variant` | warning | macOS filesystem created a numbered `.gsd` collision variant (e.g. `.gsd 2`) — can cause GSD state to appear deleted | Yes |
...
| `stale_uncommitted_changes` | warning | Uncommitted changes with no commit above the configured threshold — auto-snapshots tracked files | Yes |
...
| Code | Severity | Description | Fixable |
|------|----------|-------------|---------|
| `db_orphaned_task` | error | Task in the database references a slice that no longer exists | No |
| `db_orphaned_slice` | error | Slice in the database references a milestone that no longer exists | No |
| `db_done_task_no_summary` | warning | Task is marked done in the database but has no summary stored | No |
| `db_duplicate_id` | error | Duplicate milestone, slice, or task ID detected in the database | No |
...
| `.gsd/gsd.db` | DB constraint and projection drift checks (engine domain) |
| `.gsd/event-log.jsonl` | Projection drift detection (compared against roadmap mtime) |
...
| `.gsd/milestones/*/M*-ROADMAP.md` | Slice checkbox updates; sanitized delimiter characters; re-rendered projections |
