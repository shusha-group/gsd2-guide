...
| Flag | Description |
|------|-------------|
| `--fix` | Enable fix mode without changing the default `doctor` subcommand (equivalent to `/gsd doctor fix`) |
...
flowchart TD
    ...
    Scan --> Struct[Structural checks<br/>Plans, dependencies,<br/>duplicate IDs, timestamps]
...
| Code | Severity | Description | Fixable |
|------|----------|-------------|---------|
| `db_unavailable` | warning | Database file exists on disk but cannot be loaded — GSD falls back to filesystem state derivation | No |
...
| File | Purpose |
|------|---------|
| `.gsd/milestones/*/M*-ROADMAP.md` | Sanitized delimiter characters in milestone titles; re-rendered projections |
