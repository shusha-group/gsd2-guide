...
| Execute task | Smart-chunked (3,000-char threshold, section boundary truncation) |
...
Replan-slice, discuss-milestone, run-UAT, gate-evaluate, and rewrite-docs do not inject KNOWLEDGE.md. Reactive-execute does not inject it at the orchestrator level, but each subagent execute-task prompt it builds does.
