...
`/gsd knowledge` appends an entry to `.gsd/KNOWLEDGE.md`. This file is injected into both the base system context and each individual unit prompt — research, planning, execution, slice completion, and more — so rules, patterns, and lessons you add here influence all future agent work without needing to repeat them.
...
2. **Derive scope** — Reads the current project state to determine the active milestone and slice. The scope is formatted as `M001/S02`, `M001`, or `global` if no milestone is active.
...
|---|-------|------|-----|-------|
| K001 | M001/S02 | Always use parameterized queries | — | manual |
...
**Base system context** — At the start of every agent session, the file is injected under a `[PROJECT KNOWLEDGE — Rules, patterns, and lessons learned]` block, appended after the GSD persona and preferences.

**Unit prompts** — The file is also inlined directly into the prompt for each unit: research, planning, task execution, slice completion, milestone completion, and more. This means agents see the knowledge both globally and in context of the specific work they're about to do.
