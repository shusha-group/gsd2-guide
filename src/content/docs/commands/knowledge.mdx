...
`/gsd knowledge` appends an entry to `.gsd/KNOWLEDGE.md`. This file is injected into the system prompt for every agent session, so rules, patterns, and lessons you add here influence all future agent work without needing to repeat them.
...
`KNOWLEDGE.md` is injected into the system prompt for every agent session — task execution, slice planning, milestone research, and everything else. This means:
...
The file is only injected if it exists. If no knowledge has been added yet, the block is simply omitted.
