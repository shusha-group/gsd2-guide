...
/gsd keys remove [provider]    Remove a key
/gsd keys test [provider]      Validate key(s) with a live API call
...
Prints a grouped dashboard of all known providers organized by category. Each row shows whether the key is configured, its source (`auth.json` or env), and a masked credential description. Backed-off providers are flagged.
...
Shows only currently configured providers. For providers with multiple keys, presents a per-key picker (or "Remove all"). Removes the key from `auth.json` and clears the environment variable.
...
Shows the current masked key, prompts for a new one, optionally validates it with a live API call before saving, then replaces the old key. OAuth credentials for the same provider are preserved. If validation fails, rotation is cancelled.
...
- [`/gsd config`](../config/) — Set API keys for external tools (lighter-weight alternative)
