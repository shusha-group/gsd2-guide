...
2. **`##` heading sections** — `## vN.N — Title` headings (only when no `<details>` blocks were found), each section collecting phase checkbox lines beneath it. Generic headings like `## Phases`, `## Milestones`, and `## Progress` are skipped.
3. **Flat list** — all checkbox phase entry lines extracted directly (produces a single `M001` milestone)

Phase entry lines support two formats: `- [x] Phase NN: Title` and `- [x] NN — Title` (em-dash or en-dash only; plain hyphens are not treated as separators). Bold markers (`**`) are stripped before matching.
...
Slices are chained sequentially — each slice depends on the previous one (`depends: [S01]` for S02, etc.), except the first slice which has no dependencies.
...
After writing, the command offers to dispatch an agent-driven review. The agent is sent the `review-migration` prompt template, interpolated with the source path, target `.gsd/` path, and preview statistics. It audits the migrated output against GSD-2 standards — checking structure, content quality, `deriveState()` round-trip integrity, and requirement statuses. It fixes minor issues in-place and reports PASS/FAIL per category.
...
- [`/gsd new-project`](../new-project/) — Start a new project from scratch instead of migrating
