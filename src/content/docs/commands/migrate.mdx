...
1. **`<details>` blocks** — `<details><summary>vN.N Title (Phases N-M)</summary>` collapsed milestone blocks, each containing phase checkbox lines
2. **`##` heading sections** — `## vN.N — Title` headings, each section collecting phase checkbox lines beneath it
...
Requirement statuses from the old format are normalized: `complete`, `completed`, `done`, and `shipped` all map to `validated`. The statuses `active`, `validated`, and `deferred` are kept as-is. All other unrecognised statuses (including `out-of-scope`) default to `active`.
...
| `.gsd/REQUIREMENTS.md` | Migrated requirements grouped by status — Active, Validated, Deferred, Out of Scope (only written if requirements exist) |
