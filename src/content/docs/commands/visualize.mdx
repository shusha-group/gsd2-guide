...
**5. Health** — Session health across multiple sections. **Budget**: visual progress bar against the configured ceiling (if set) and the active token profile. **Pressure**: truncation rate and continue-here rate as color-coded bars (green/yellow/red). **Routing**: tier breakdown with downgrade counts and savings summary. **Session**: raw tool call and message counts. **Environment**: any failing environment checks (missing tools, configuration issues) surfaced from the doctor checks. **Providers**: status of configured LLM, notification, search, and tool providers. **Progress Score**: traffic-light status (green/yellow/red) with positive and negative signals, shown only when auto-mode has health history. **Doctor History**: the last 10 persisted doctor run entries (up to 20 stored), with timestamps, scope tags, error/warning/fix counts, issue details, and fix descriptions. **Skills**: a summary of installed skills with warning/critical counts and the top issue, with a link to `/gsd skill-health` for the full report.
...
- **Doctor history** — `.gsd/doctor-history.jsonl` for the persisted doctor run log shown in the Health tab.
- **Progress score** — In-memory health history from the running auto-mode session, used to compute the traffic-light Progress Score in the Health tab.
...
| `.gsd/doctor-history.jsonl` | Persisted doctor run history for the Health tab |
...
| `.gsd/export-<timestamp>.md` | Written when **[m]** Markdown export is triggered |
| `.gsd/export-<timestamp>.json` | Written when **[j]** JSON export is triggered |
...
Checking the Health tab for environment, provider status, and progress score:
...
> press 5
  ...
  Progress Score
    ● Healthy — execution on track
    ✓ Low truncation rate
    ✓ No blockers discovered
    · 2 slices remaining

  Doctor History
    ✓ 2025-01-15 14:30:00  0 errors, 0 warnings, 0 fixes
    ✗ 2025-01-14 09:12:00  1 errors, 2 warnings, 1 fixes
      ✗ missing required prompt permission on gsd:execute-phase
      ↳ added prompt permission to settings
...
- [`/gsd export`](../export/) — Generate a shareable HTML report snapshot from the same data pipeline
- [`/gsd capture`](../capture/) — Add a capture that appears in the Captures tab
- [`/gsd skill-health`](../skill-health/) — Full skill metrics dashboard (linked from the Health tab)
