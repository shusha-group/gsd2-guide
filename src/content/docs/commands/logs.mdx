...
- **Debug logs** (`.gsd/debug/debug-*.log`) — Internal timing and dispatch diagnostics. Only written when `GSD_DEBUG=1` is set.
...
flowchart TD
    ...
    DebugFlag(["GSD_DEBUG=1"]) --> DL[Debug logs\n.gsd/debug/debug-*.log]
...
Debug logging is off by default. Enable it with:
...
- Any error or failure events captured
...
| `.gsd/debug/debug-<timestamp>.log` | Debug logs | When `GSD_DEBUG=1` is set |
