...
flowchart TD
    ...
    H --> I[Check if milestone is complete<br/>via ROADMAP]
    I --> J[Force-delete stale<br/>milestone branches]
    J --> K[Report summary]
...
2. **Stale milestone branches** — Lists all `milestone/*` branches and cross-references the currently registered worktrees. Any `milestone/*` branch that is **not** attached to a live worktree and whose milestone is marked complete in the roadmap is force-deleted. These accumulate when parallel milestone work finishes and the worktree is removed but the branch lingers.
...
A worktree that is currently your working directory is always skipped, even if it would otherwise be safe to remove.

### Project State Cleanup

GSD stores per-project planning state in `~/.gsd/projects/<hash>/` directories, keyed by a hash of the project's git remote URL. When a project is deleted or moved, its state directory becomes orphaned.

`/gsd cleanup projects` (dry-run) inspects each directory, reads its `META.json`, and checks whether the recorded git root path still exists on disk:
...
| `.gsd/milestones/<MID>/<MID>-ROADMAP.md` | Checks milestone completion before deleting stale milestone branches |
| `~/.gsd/projects/<hash>/META.json` | Reads git root path to detect orphaned state directories |
...
- [`/gsd parallel`](../parallel/) — Manage parallel milestone worktrees (start, status, merge, stop)
