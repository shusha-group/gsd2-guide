...
flowchart TD
    ...
    CheckPending -->|None| CheckPending3{Pending milestones?}
    CheckPending3 -->|Yes| QueuedPicker
    CheckPending3 -->|No| NoPending([All slices complete — nothing to discuss])
    ...
    QueuedPicker --> HasDraft{Has CONTEXT-DRAFT?}
    HasDraft -->|Yes| QueuedDiscuss[Milestone interview with draft seed]
    HasDraft -->|No| FastPathChoice{Full or fast path?}
    FastPathChoice -->|Full discussion| QueuedDiscuss
    FastPathChoice -->|Fast path| QueuedDiscuss
    ...
    style CheckPending3 fill:#0d180d,stroke:#39ff14,color:#39ff14
    style Select fill:#0d180d,stroke:#39ff14,color:#39ff14
    style More fill:#0d180d,stroke:#39ff14,color:#39ff14
    style HasDraft fill:#0d180d,stroke:#39ff14,color:#39ff14
    style FastPathChoice fill:#0d180d,stroke:#39ff14,color:#39ff14
...
When a queued milestone already has a `CONTEXT-DRAFT.md`, the discussion seeds itself automatically from that draft. When there is no draft, you are offered two modes:
...
| File | Purpose |
|------|---------|
| `.gsd/milestones/<MID>/slices/<SID>/<SID>-CONTEXT-DRAFT.md` | Incremental draft saved every 2 question rounds to protect against crashes |
