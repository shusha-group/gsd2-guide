---
title: "/gsd discuss"
description: "Run a guided interview about an upcoming slice or queued milestone — capture UX, behaviour, and scope decisions before auto mode plans it."
---
...
No flags. Requires an active milestone with a roadmap and at least one pending slice, or a pending milestone to discuss.
...
flowchart TD
    ...
    CheckMilestone -->|No| CheckPending2{Pending milestones?}
    CheckPending2 -->|No| NoMilestone([No milestone — run /gsd to create one])
    CheckPending2 -->|Yes| QueuedPicker[Show queued milestone picker]
    QueuedPicker --> QueuedDiscuss[Milestone interview — no auto-start]
    ...
    CheckPhase -->|normal| CheckRoadmap{Roadmap / slices?}
    ...
    Select -->|Slice chosen| AlreadyDiscussed{Already has CONTEXT?}
    Select -->|Discuss queued milestone| QueuedPicker
    AlreadyDiscussed -->|Yes| ReDiscussConfirm[Confirm re-discuss]
    ReDiscussConfirm -->|Cancel| Picker
    ReDiscussConfirm -->|Re-discuss| Interview
    AlreadyDiscussed -->|No| Interview[Guided slice interview]
    ...
    style AlreadyDiscussed fill:#0d180d,stroke:#39ff14,color:#39ff14
    style CheckRoadmap fill:#0d180d,stroke:#39ff14,color:#39ff14
    style CheckPending fill:#0d180d,stroke:#39ff14,color:#39ff14
    style CheckPending2 fill:#0d180d,stroke:#39ff14,color:#39ff14
    ...
    style ReDiscussConfirm fill:#1a3a1a,stroke:#39ff14,color:#e8f4e8
    ...
    style QueuedPicker fill:#1a3a1a,stroke:#39ff14,color:#e8f4e8
    style QueuedDiscuss fill:#1a3a1a,stroke:#39ff14,color:#e8f4e8
...
The picker loops: after each discussion finishes, it returns to the picker so you can chain multiple slice interviews in one session. When all pending slices have been discussed, it exits.
...
When there is no active milestone (all are complete or none exist yet), `/gsd discuss` shows a picker of pending milestones from the registry. Selecting one dispatches the `guided-discuss-milestone` prompt for that milestone.
...
| `.gsd/milestones/<MID>/slices/<SID>/<SID>-CONTEXT.md` | Existing slice context (for re-discuss flow) |
...
| `.gsd/milestones/<MID>/<MID>-CONTEXT.md` | Written when discussing a milestone (needs-discussion or queued flow) |
| `.gsd/DISCUSSION-MANIFEST.json` | Written during multi-milestone discussions to track gate completion |

### Writes

| File | Purpose |
|------|---------|
| `.gsd/milestones/<MID>/slices/<SID>/<SID>-CONTEXT.md` | Slice context from the interview; rewritten on re-discuss |
| `.gsd/milestones/<MID>/<MID>-CONTEXT.md` | Updated when a milestone-level re-discussion concludes |
...
Discussing a queued milestone without activating it:
...
> M002: Analytics & Reporting
  ...
  [Agent runs milestone interview for M002 without changing active milestone]
  ✓ M002 context written.
```

## Prompts Used

- [`guided-discuss-slice`](../../prompts/guided-discuss-slice/) — Slice interview prompt (normal flow)
- [`guided-discuss-milestone`](../../prompts/guided-discuss-milestone/) — Milestone interview prompt (needs-discussion and queued milestone flows)
