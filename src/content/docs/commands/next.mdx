...
flowchart TD
    ...
    MapPhase --> LoadMetrics["Load ledger + preferences"]
...
2. **Checks for an active milestone** — If there's no active milestone, it exits with "nothing to dispatch."
3. **Maps phase to unit type** — Uses a direct phase-to-unit-type mapping (not the full dispatch loop):

| Phase | Unit type dispatched |
|-------|----------------------|
...
| `blocked` | `blocked` |
| any other phase | phase name verbatim |

4. **Loads historical metrics** — Reads the in-memory ledger (or falls back to the ledger on disk) to calculate average cost and duration for units of the same type. Loads preferences to check the budget ceiling.
...
| File | Purpose |
|------|---------|
| `.gsd/<MID>/ROADMAP.md` | Read to derive active milestone and phase |
| `.gsd/<MID>/<SID>/PLAN.md` | Read to derive active slice and task |
| `.gsd/<MID>/<SID>/SUMMARY.md` | Read to determine slice completion |
| `.gsd/ledger.json` | Read for historical unit cost and duration averages |
| GSD preferences file | Read for budget ceiling (`budget_ceiling`) |
