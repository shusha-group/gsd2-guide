...
The behavior differs depending on whether auto-mode is running. When active, the override is stored and the dispatch loop's first rule fires a `rewrite-docs` unit at the next cycle — that unit updates all active plan documents and marks the override resolved. When inactive, a `gsd-hard-steer` message instructs the agent to update plans directly in the current conversation.
...
3. **Prompt injection** — Active overrides (scope `active`) are automatically injected into every future task prompt as an "Active Overrides" section that supersedes any conflicting plan content. This injection happens through the system context builder for each dispatched task.
...
After the `rewrite-docs` unit runs, `**Scope:** active` is changed to `**Scope:** resolved`. The `rewrite-docs` unit itself performs this change as its final step — once no `**Scope:** active` entries remain, the dispatch loop moves on to the next task.
...
- Triggers a **`rewrite-docs` unit** at the next dispatch cycle — `rewrite-docs` is the first rule in the dispatch table and fires whenever active overrides exist, reading `OVERRIDES.md` and propagating changes across all active plan documents before any other work runs

The `rewrite-docs` unit updates these document types based on what the override affects:
...
The milestone `CONTEXT.md` file is listed as reference-only and is never modified by `rewrite-docs`.

A circuit breaker limits rewrite attempts to 3 per override set — if the unit fails to complete after 3 tries, all active overrides are force-resolved (a regex replace of `**Scope:** active` → `**Scope:** resolved` across the entire file) to prevent an infinite rewrite loop.
