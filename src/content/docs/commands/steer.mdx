...
| `.gsd/runtime/rewrite-count.json` | Circuit breaker attempt counter read at each dispatch cycle |
...
| `.gsd/runtime/rewrite-count.json` | Incremented each time the override gate fires; reset to zero after successful rewrite-docs completion |
