...
flowchart TD
    ...
    Dispatch[Evaluate 19 dispatch rules] --> Execute[Execute unit in fresh session]
...
1. **Dispatch** — Evaluates a declarative table of 19 ordered dispatch rules. The first matching rule determines the unit type and builds the prompt for that unit.
...
7. **Sidecar drain** — After each primary unit, the sidecar queue is drained. Sidecars are lightweight units enqueued during post-unit processing: post-unit hooks (`hook/<name>`), triage captures (`triage-captures`), and quick-task captures queued after triage.
