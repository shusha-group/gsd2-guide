...
flowchart TD
    ...
    Crash -->|No| DB
    Recover --> DB[Open or auto-migrate SQLite DB]
    DB --> Audit[Audit orphaned milestone branches]
    Audit --> Derive[Derive project state from disk]
    Derive --> HasMilestone{Milestone exists?}
    HasMilestone -->|No| Discuss[discuss-milestone wizard]
    HasMilestone -->|Yes| IsoMode
    Discuss --> IsoMode{Worktree isolation enabled?}
    IsoMode -->|Yes| Worktree[Create or re-enter worktree]
    IsoMode -->|No| Metrics
    Worktree --> Metrics[Init metrics, routing history & skill snapshot]
    ...
    Dispatch[Evaluate 20 dispatch rules] --> Execute[Execute unit in fresh session]
    ...
    style IsoMode fill:#0d180d,stroke:#39ff14,color:#39ff14
    ...
    style Audit fill:#1a3a1a,stroke:#39ff14,color:#e8f4e8
...
7. **Derive state** — Reads all `.gsd/` files to determine the current phase: which milestone is active, which slice, which task. This is the same algorithm used by [`/gsd status`](../status/).
...
| `triage-captures` | sidecar | Classifies pending captures at natural seams between units; see [`/gsd capture`](../capture/) |
...
When `phases.require_slice_discussion` is enabled, auto mode pauses before each new slice that does not yet have a slice context file, prompting you to run [`/gsd discuss`](../discuss/) before planning begins. This gives you a checkpoint to review slice scope before the planner runs.
...
- **Remediation verdict guard** — `completing-milestone` checks the VALIDATION file verdict. If the verdict is `needs-remediation`, GSD stops rather than completing — remediation work must be addressed first. (Note: the state machine treats `needs-remediation` as terminal to prevent validation loops, but completion is blocked regardless.)
...
The selected mode is persisted to preferences so it survives restarts. You can also set the mode directly with [`/gsd widget <mode>`](../widget/).
...
| `.gsd/runtime/auto.lock` | Crash lock file with PID and current unit |
| `.gsd/runtime/session.lock` | Session lock tracking active PID and unit for cross-process coordination |
...
- [`/gsd pause`](../pause/) — Pause auto mode, preserving state for resumption
...
- [`/gsd widget`](../widget/) — Change the progress widget display mode
- [`/gsd discuss`](../discuss/) — Discuss a slice before planning (required when `require_slice_discussion` is enabled)
