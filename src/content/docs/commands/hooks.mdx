...
**Agent override:** Each hook can optionally specify an `agent` field pointing to a custom agent definition file, and a `model` field to override the model used for that hook session.
...
No hooks configured. Add post_unit_hooks or pre_dispatch_hooks to .gsd/PREFERENCES.md
```
