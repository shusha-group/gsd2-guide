...
The command calls `formatHookStatus()` from the hook engine facade, which delegates to the `RuleRegistry` singleton via `getOrCreateRegistry()`. The registry calls `getHookStatus()` to collect all configured hooks and their live cycle counts, then formats them for terminal display.
