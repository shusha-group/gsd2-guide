---
title: "/gsd config"
description: "Configure API keys for external tool integrations — search providers, document extractors, and voice."
---
...
flowchart TD
    ...
    Changed -->|yes| Wait[Wait for idle]
    Wait --> Reload[Reload extensions]
    Reload --> End([Done])
    Changed -->|no| End
...
6. **Reload** — Once the loop exits, if any keys changed, GSD waits for the agent to be idle then reloads its extensions so tools pick up the new credentials immediately.
