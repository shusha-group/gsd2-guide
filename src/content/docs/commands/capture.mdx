...
When captures accumulate past a threshold, triage fires automatically at the next natural seam between tasks — no manual intervention needed. The progress widget shows a pending capture count badge so you can see how many are waiting.
...
- **Direct layout**: `/.gsd/worktrees/<MID>/`
...
**Triage only classifies — it does not execute resolutions.** After the triage unit completes, auto-mode calls `executeTriageResolutions()` at the next dispatch cycle to carry out the actions:

- **`inject`** — Appends a new task (with the next available task ID, e.g. `T05`) to the current slice's `PLAN.md`, before the "Files Likely Touched" section. The injected task includes a `Why`, `Do`, and `Done when` scaffold, and an `est:30m` estimate.
- **`replan`** — Writes a `<SID>-REPLAN-TRIGGER.md` marker file in the slice directory. The next dispatch cycle detects this and enters the replanning-slice phase.
- **`quick-task`** — Collected for dispatch as self-contained units at the next natural seam. The agent executes the task without modifying any plan files.
- **`defer`** — If the target milestone doesn't exist yet, creates its directory and seeds a `<MID>-CONTEXT-DRAFT.md` with the deferred captures listed. This ensures `deriveState()` discovers the milestone and routes to the discussion phase.
...
| `.gsd/milestones/<MID>/<MID>-CONTEXT-DRAFT.md` | Seeded when a `defer` target milestone doesn't exist yet |
...
| `.gsd/milestones/<MID>/<MID>-ROADMAP.md` | Read during triage for classification context |
