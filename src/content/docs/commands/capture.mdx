...
When a unit completes in auto-mode and there are any pending captures, triage fires automatically at the next seam — before the next unit is dispatched. The triage is enqueued as a sidecar unit on the internal `sidecarQueue`.

Auto-triage only fires when all of these conditions are met:
- Auto-mode is not in step mode
- The completing unit is not itself a `triage-captures` or `quick-task` unit (nor a hook)
- There is an active milestone AND an active slice — triage needs plan context to classify captures
...
After triage, the status is updated and classification fields are appended — including the milestone the capture was triaged in:
...
- **`quick-task`** — A dedicated prompt (`buildQuickTaskPrompt`) is generated from the capture text and collected for dispatch. The capture is marked as executed immediately when queued. The agent then executes the task without modifying any plan files.
...
Captures are resolved in the context of a specific milestone — the `**Milestone:**` field records which one. When `executeTriageResolutions` runs, it only processes captures resolved in the **current** milestone.

This prevents quick-task captures from a prior milestone re-executing after the underlying issues were already fixed by planned milestone work. Captures that have no `**Milestone:**` field (legacy captures resolved before this field was introduced) are always included for backward compatibility.
...
| File | Purpose |
|------|---------|
| `.gsd/CAPTURES.md` | New capture entry appended; status, classification, and milestone fields updated after triage |
