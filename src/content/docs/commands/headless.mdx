...
|------|--------|---------|-------------|
| `--timeout` | `--timeout N` | `300000` (5 min) | Overall timeout in milliseconds. The process exits with code `1` if this limit is reached. `new-milestone` auto-extends this to `600000` (10 min). |
...
The timeout is automatically extended to 10 minutes for `new-milestone` commands (regardless of the `--timeout` default) because milestone creation involves codebase investigation and writing multiple planning artifacts.
