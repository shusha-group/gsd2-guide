...
When you have more than one pending milestone (active or queued), the hub presents two options: reorder the existing queue or add new work. If only one milestone exists, it skips straight to the add-new-work flow.
...
The command reads the full project state via `deriveState()`, which scans the milestone registry to determine which milestones are complete, active, pending, or parked. It uses this to build the list of reorderable items and to provide context to the add-flow LLM.

### Reorder Flow

The reorder UI shows completed milestones (dimmed) and all pending milestones in their current execution order. Use the keyboard to navigate and reposition items:
...
| `d` | Manually remove a `depends_on` from the cursor item |
...
2. **Dependency cleanup** — Any `depends_on` entry that a reorder has made contradictory (a milestone now appears before its dependency) is flagged as `would_block` and automatically removed from the milestone's `CONTEXT.md` frontmatter on confirm. The UI shows these in orange before you hit Enter so you know what will be cleaned up.
...
No roadmap is created — roadmaps are generated just-in-time when auto-mode reaches that milestone.
...
| `.gsd/milestones/*/MXXX-CONTEXT.md` | `depends_on` removed when reordering creates `would_block` conflicts |
