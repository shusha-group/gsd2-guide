...
When you have more than one non-complete, non-parked milestone (status `active` or `pending`), the hub presents three options: reorder the existing queue, add new work, or dismiss for now. If only one such milestone exists, it skips straight to the add-new-work flow.
...
flowchart TD
    ...
    Milestones -->|Yes| Count{Active or pending\nmilestones > 1?}
...
4. **Verifies before writing** — For each new milestone, a mandatory depth verification step presents the proposed scope and any technical risks. You confirm or correct before the context file is written. This gate is mechanically enforced — the system blocks `CONTEXT.md` writes until depth verification passes and cannot be skipped.
5. **Writes artifacts** — Uses `gsd_milestone_generate_id` to get a fresh ID (never invented manually), creates `.gsd/milestones/<ID>/slices/`, and writes `<ID>-CONTEXT.md`. If the new work has dependencies on other milestones, YAML frontmatter with `depends_on` is added to the context file.
...
| `.gsd/milestones/*/<ID>-CONTEXT.md` | Full context for each milestone (dedup and scope checks) |
| `.gsd/milestones/*/<ID>-CONTEXT-DRAFT.md` | Draft seed files surfaced during add flow; also used as fallback when CONTEXT.md is absent |
| `.gsd/milestones/*/<ID>-ROADMAP.md` | Roadmap for active/pending/parked milestones (shows what's planned) |
| `.gsd/milestones/*/<ID>-SUMMARY.md` | Read by state derivation to determine milestone completion status |
