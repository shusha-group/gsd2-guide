...
flowchart TD
    ...
    GenReport --> FilterDecline[Filter skills table to flagged only]
    FilterDecline --> DashDecline([Render report with<br/>flagged skills only in table])

    Route -->|--stale N| GenReport2[Generate health report<br/>with custom threshold]
    GenReport2 --> FilterStale[Detect stale skills via threshold]
    FilterStale --> DashStale([Render full dashboard<br/>with stale section])
...
The `--declining` flag filters the full dashboard to show only flagged skills in the table. The stale, declining, and heal suggestion sections are still included below the table. If no skills are flagged, it prints `No skills flagged for declining performance.`
...
- **Declining success** — `Success rate dropped to X% over Y uses. Review SKILL.md for outdated patterns.` (warning or critical based on severity)
- **Rising tokens** — `Token usage trending upward. Skill may be causing inefficient execution patterns.` (info; requires 10+ uses)
- **Stale** — `Not used in 60+ days. Consider archiving or updating.` (info)
...
- **None** — agent followed the skill correctly → writes "No drift detected" to the unit heal artifact and stops
...
If multiple issues are found in a single unit, one entry is written per issue. The prompt explicitly instructs the agent not to flag stylistic preferences — only genuine errors or outdated content.
...
Declining-only view (filters the table to flagged skills; stale/declining/suggestion sections still appear):
