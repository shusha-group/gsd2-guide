...
- **Token trend rising 20%+** — recent average tokens are significantly higher than the previous window

The `--declining` flag filters the full dashboard to show only flagged skills.
...
- **Rising tokens** — `Skill may be causing inefficient execution patterns` (info)
...
After each auto-mode unit, GSD can optionally run a heal-skill analysis. It reads the skill that was loaded, compares actual execution against the skill's guidance, and writes a structured proposal to `.gsd/skill-review-queue.md` — one entry per issue found. The agent never modifies a skill file directly. This human review step is intentional: research shows that curated skills outperform auto-generated ones by +16.2pp.
