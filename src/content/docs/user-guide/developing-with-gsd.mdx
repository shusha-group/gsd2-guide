...
For milestones that depend on others, GSD writes `depends_on` frontmatter into each `CONTEXT.md`:
...
Auto mode is a **dispatch state machine** — a declarative table of rules evaluated in order. It reads the `.gsd/` directory structure to determine what phase the project is in, then the first matching rule fires:
...
flowchart TD
    ...
    Check -->|needs-discussion| Stop1[Stop: run /gsd to discuss]
    Check -->|pre-planning| NeedCtx{Has context?}
    NeedCtx -->|No| Stop2[Stop: run /gsd to discuss]
    ...
    Check -->|summarizing| NeedUAT{Needs UAT?}
    NeedUAT -->|Yes| UAT[run-uat]
    NeedUAT -->|No| NeedRA{Needs reassess?}
    NeedRA -->|Yes| RA[reassess-roadmap]
    NeedRA -->|No| CS[complete-slice]
    ...
    style NeedUAT fill:#0d180d,stroke:#39ff14,color:#39ff14
    style NeedRA fill:#0d180d,stroke:#39ff14,color:#39ff14
    ...
    style Stop2 fill:#3a1a1a,stroke:#ff4444,color:#e8e8e8
```
...
After all tasks are done, GSD runs through a sequence of post-completion steps:

1. **Run UAT** — if `uat_dispatch` is enabled and a UAT script exists, executes the acceptance test script
2. **Reassess roadmap** — checks whether the remaining slices still make sense given what was built; may reorder, merge, or drop slices (can be skipped via `skip_reassess` preference)
3. **Complete slice** — writes the slice summary, compresses all task summaries, marks the slice done in the roadmap

The completion agent:
- Runs all slice-level verification — the objective stopping condition defined in the slice plan
- Fixes failures before proceeding
- Writes `S01-SUMMARY.md` — compressed slice record
...
When all slices are done, GSD enters `validating-milestone` before writing the milestone summary. The validation agent:
...
If remediation is needed and remediation slices exist in the roadmap, GSD executes them before re-validating. Once the validation is terminal (`pass` or `needs-attention`), GSD proceeds to completion.
...
1. Writes `M001-SUMMARY.md` — the milestone-level record
2. Updates `PROJECT.md` with current state
3. If another milestone exists (M002), GSD continues into its discussion/research cycle
...
2. Check milestone-level `depends_on` frontmatter in each `CONTEXT.md` — skip milestones whose dependencies aren't complete
3. The first incomplete, unblocked milestone becomes the **active milestone**
4. Within that milestone, check if all slices are done and whether a `VALIDATION` file with a terminal verdict exists
5. If all slices done but no terminal validation → phase is `validating-milestone`
6. If all slices done and validation terminal but no summary → phase is `completing-milestone`
7. Otherwise, find the first incomplete slice (with dependencies met) → **active slice**
8. If the slice has no plan → phase is `planning`
9. If all tasks done but slice not marked complete → phase is `summarizing`
10. If a completed task has `blocker_discovered: true` and no `REPLAN.md` → phase is `replanning-slice`
11. Otherwise → phase is `executing` with the first incomplete task as the **active task**

### All phases

| Phase | What it means |
|-------|--------------|
| `pre-planning` | Active milestone has context but no roadmap yet |
...
| `summarizing` | All tasks done — run UAT, reassess, complete slice |
| `replanning-slice` | A task discovered a blocker — replan remaining tasks |
...
| `blocked` | No eligible slice (dep cycle) or all milestones dep-blocked |
...
State derivation uses a native Rust batch parser when available, reading all `.gsd/` markdown files in a single pass for speed. It falls back to sequential JS file reads if the native module is absent.
...
For work that doesn't fit into the milestone/slice ceremony, use `/gsd quick <description>`. Quick tasks live in `.gsd/quick/` and are tracked in `STATE.md`. They get GSD's execution guarantees (atomic commits, verification) without a full slice plan.
...
| **Blocker replanning** | Tasks that discover plan-invalidating issues trigger automatic slice replanning |
