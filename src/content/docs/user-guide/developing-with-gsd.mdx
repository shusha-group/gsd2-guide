...
### Depth verification gate

Before GSD writes any files, it presents a structured depth summary and asks you to confirm. The system mechanically blocks `CONTEXT.md` writes until you approve — GSD cannot rationalize past this gate. This ensures the context file captures your actual intent before planning begins.

### What discussion produces

When discussion finishes, GSD writes several artifacts:
...
- **`DECISIONS.md`** — architectural decisions made during discussion, auto-assigned IDs via `gsd_decision_save`
...
The context file is the authoritative record of what this milestone should achieve. Every downstream phase reads it. Once `STATE.md` is written, auto-mode starts automatically and runs through research and planning.
...
Research happens automatically when auto-mode starts (triggered after discussion completes). The research agent:
...
Milestone-level research is skipped for S01 (the first slice uses it directly). Subsequent slices get their own focused slice research unless `phases.skip_slice_research` is set in preferences.
...
After research completes, the milestone planner (`plan-milestone`) reads the context and research, then decomposes work into **slices** — demoable vertical increments ordered by risk:
...
The roadmap also includes: key risks and unknowns, proof strategy (which slice retires which risk), verification classes (contract, integration, operational, UAT), milestone definition of done, requirement coverage, a boundary map of what each slice produces and consumes, and a horizontal checklist of cross-cutting concerns (auth boundaries, reconnection strategies, billing path impact) to check across all slices.
...
flowchart TD
    ...
    Check -->|planning| SR{Active slice\nneeds research?}
    SR -->|"Yes (not S01)"| RS[research-slice]
    SR -->|"No, but 2+ slices\nready for research"| PRE[parallel-research-slices]
    SR -->|No| PS[plan-slice]

    Check -->|evaluating-gates| GE{Gates enabled?}
    GE -->|Yes| EG[gate-evaluate]
    GE -->|No| Skip[skip]
    ...
    style PRE fill:#1a3a1a,stroke:#39ff14,color:#e8f4e8
    ...
    style GE fill:#0d180d,stroke:#39ff14,color:#39ff14
    style EG fill:#1a3a1a,stroke:#39ff14,color:#e8f4e8
...
When `gate_evaluation.enabled: true` is set in preferences, GSD enters `evaluating-gates` after a slice plan is written. The `gate-evaluate` agent runs configured quality gates (by default Q3 and Q4) against the slice plan before any execution begins. If no gates are pending or gate evaluation is disabled, this phase is skipped automatically and execution proceeds immediately.

Task-level gates (Q5: Failure Modes, Q6: Load Profile, Q7: Negative Tests) are embedded directly into task plans when the gate-evaluate agent flags them. Executors implement the specified handling as part of their task steps.

### Parallel slice research

When a slice's research is already done but multiple upcoming slices simultaneously have their dependencies met and need research, GSD dispatches them in a single `parallel-research-slices` unit instead of researching each one sequentially. This is an automatic optimization — no configuration required. If only one slice is ready, GSD researches it sequentially as normal.
...
├── CODEBASE.md                  ← generated codebase map cache (auto-refreshed)
├── OVERRIDES.md                 ← active /gsd steer overrides (if any)
├── QUEUE.md                     ← queued milestones log (if any)
├── runtime/                     ← system-managed: dispatch state, do not edit
├── activity/                    ← system-managed: JSONL execution logs, do not edit
├── worktrees/                   ← system-managed: auto-mode worktree checkouts, do not edit
└── milestones/
    └── M001/
        ├── M001-CONTEXT.md
        ├── M001-RESEARCH.md
...
3. **UAT verdict gate** — if UAT ran and the verdict is not `PASS`, auto-mode stops. You must fix the failures and re-run `/gsd auto` to continue. If run-uat is dispatched 3 times without producing a verdict (e.g., broken verification commands), auto-mode stops with an error instead of looping indefinitely.
4. **Reassess roadmap** — if `phases.reassess_after_slice: true` is set in preferences, checks whether the remaining slices still make sense given what was built; may reorder, merge, or drop slices. Also skipped when `phases.skip_reassess: true` is set.
...
When `phases.reassess_after_slice: true` is set in preferences, GSD reassesses the remaining roadmap after each slice completes. Plans made before execution may not survive contact with reality:
...
The reassessment either confirms the plan or adjusts it. This is how GSD adapts without losing structure. Reassessment is opt-in — it does not run by default.
...
Set `phases.skip_milestone_validation: true` in preferences to write a pass-through validation file and proceed directly to completion. Useful for projects where the milestone definition of done is verified by other means.
:::
...
2. Skip **ghost milestones** — directories with no substantive files (`CONTEXT`, `CONTEXT-DRAFT`, `ROADMAP`, or `SUMMARY`). A ghost is NOT skipped if it has a non-queued DB row or a worktree directory. Queued DB rows without any content files are still treated as ghosts.
...
| `discussing` | A discuss-milestone unit is actively running |
| `researching` | A research-milestone or research-slice unit is actively running |
| `planning` | Active slice needs a plan |
| `evaluating-gates` | Slice plan written — quality gates being evaluated before execution |
| `executing` | Active task is ready to run |
| `verifying` | Post-task verification gate is running |
| `summarizing` | All tasks done — complete slice, then UAT and reassess |
| `advancing` | Slice completed — advancing to the next unit (UAT, reassess, or next slice) |
...
| `paused` | Auto-mode manually paused by `/gsd pause` |
...
State derivation is DB-primary: when a `gsd.db` SQLite database is present (created during project init), milestone and slice data is read from tables directly, keeping derivation fast even for large projects. Filesystem parsing serves as a fallback for unmigrated projects or when the DB is empty — both paths produce field-identical `GSDState` output. On a disk-only project, a native Rust batch parser is used when available to read all `.gsd/` markdown files in a single pass. Results are cached for 100ms per base path to avoid redundant re-reads within a single dispatch cycle.
...
- **`/gsd steer`** — injects a steering override that the next unit will incorporate; written to `OVERRIDES.md`
...
| **UAT verdict gate** | A non-PASS UAT result blocks auto-mode until you fix the failures; 3 attempts without a verdict also stops auto-mode |
...
| **Quality gate evaluation** | After slice planning, configured gates run against the plan before execution starts |
| **DB-primary state** | Milestone and slice data is stored in `gsd.db`; filesystem parsing is the fallback |
| **Blocker replanning** | Tasks or triage that discover plan-invalidating issues trigger automatic slice replanning |
| **Roadmap reassessment** | Opt-in (`phases.reassess_after_slice: true`) — re-evaluates remaining slices after each slice completes |
| **Parallel slice research** | When 2+ slices are ready for research simultaneously, GSD researches them in a single parallel unit |
...
| **CODEBASE.md** | Generated codebase map cache in `.gsd/` root — auto-refreshed when tracked files change |
| **Slice discussion gate** | Set `phases.require_slice_discussion: true` to pause auto-mode before each slice for a focused discussion |
