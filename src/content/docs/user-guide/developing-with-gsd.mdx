...
4. **Reassess roadmap** — if `reassess_after_slice: true` is set in preferences, checks whether the remaining slices still make sense given what was built; may reorder, merge, or drop slices. Also skipped when `skip_reassess: true` is set.
...
When `reassess_after_slice: true` is set in preferences, GSD reassesses the remaining roadmap after each slice completes. Plans made before execution may not survive contact with reality:
...
2. Skip **ghost milestones** — directories containing only `META.json` with no substantive files (`CONTEXT`, `ROADMAP`, or `SUMMARY`), unless a DB row or worktree exists for the milestone
...
| **Roadmap reassessment** | Opt-in (`reassess_after_slice: true`) — re-evaluates remaining slices after each slice completes |
