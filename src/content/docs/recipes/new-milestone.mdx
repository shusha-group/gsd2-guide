...
5. **Questioning** — Adaptive Q&A as a thinking partner. GSD probes vague answers, challenges abstract claims, and follows your energy. It uses position-first framing ("I'd lean toward X because Y — does that match your thinking?"), asks for negative constraints ("what should it never feel like?"), and weaves your exact terminology back into subsequent questions.
6. **Depth verification** — A structured checkpoint before wrapping up. GSD prints a synthesis in chat — your exact terminology, the constraints you emphasized, the trade-offs you shaped — and asks for confirmation. This is the write gate. GSD will not write any files until depth verification passes.
7. **Roadmap preview** — After depth verification, GSD produces or updates `REQUIREMENTS.md`, then prints the planned roadmap as a markdown table in your terminal. One row per slice: Slice, Title, Risk, Depends, Demo. You approve the structure before anything is written to disk.

### Capability Contract

`REQUIREMENTS.md` is produced or updated on every discussion — not just the first one. Before writing a roadmap, GSD presents candidate requirements in a chat table with columns: ID, Title, Status, Owner, Source. Groups by status (Active, Deferred, Out of Scope). You confirm, adjust, or add before they're locked.

For new projects (or any project without an existing `REQUIREMENTS.md`), GSD runs a focused research pass first. It surfaces:
...
Research findings are presented as candidate requirements — not silently turned into scope.

### Auto-Start After Discussion

After writing all artifacts, auto-mode starts automatically — no need to run `/gsd auto` separately. The transition is seamless: discuss, confirm, and auto-mode picks up immediately.
...
- **Discuss now** — Run a focused discussion for this milestone in the current session while context is still fresh. Before writing `CONTEXT.md`, GSD performs a mandatory technical assumption verification — reading actual code files, checking APIs exist, and presenting findings with a per-milestone depth verification gate. Writes a full `CONTEXT.md` when confirmed.
...
- **Planning** decomposes the milestone into demoable vertical slices ordered by risk. Each slice is real, shippable product progress — not a foundation layer or spike. If the milestone has only one slice, the planner also writes the slice plan and task plans inline, skipping a separate plan-slice cycle.
...
| `.gsd/REQUIREMENTS.md` | Updated with requirements from the new milestone's discussion (when file already exists) |
...
After discussion and depth verification, GSD prints the requirements table then the roadmap preview:

```
● Requirements for M002: Social Features

  | ID   | Title                  | Status | Owner | Source |
  |------|------------------------|--------|-------|--------|
  | R001 | Follow system          | Active | S01   | user   |
  | R002 | Activity feed          | Active | S02   | user   |
  | R003 | Recipe comments        | Active | S03   | user   |
  | R004 | Shareable recipe links | Active | S04   | user   |
...
flowchart TD
    ...
    QA --> DepthCheck[Depth verification — write gate]
    DepthCheck --> ReqPass[Requirements table + roadmap preview]
