...
```
/gsd:quick [--full] <task description>
```

Quotes are optional. If no description is provided, GSD will prompt you interactively.
...
The `--full` flag enables plan-checking (up to 2 revision iterations) and post-execution verification via gsd-verifier. Use it when you want quality guarantees without full milestone ceremony.

## How It Works

### Execution Flow

1. **Parse arguments** — Strips `--full` flag if present. If no description remains, prompts you interactively.
...
4. **Spawn planner** — Dispatches to gsd-planner (quick mode) with your description and STATE.md as context. Planner creates a focused `<N>-PLAN.md` with 1–3 atomic tasks.
5. **(--full) Plan-checker loop** — gsd-plan-checker validates the plan. If issues are found, the planner revises — up to 2 iterations. After 2 failed iterations, you can force-proceed or abort.
6. **Spawn executor** — gsd-executor reads the plan and STATE.md, implements the work, commits each task atomically, and writes `<N>-SUMMARY.md`.
7. **(--full) Verification** — gsd-verifier checks the `must_haves` in the plan against the actual codebase and writes `<N>-VERIFICATION.md`.
8. **Update STATE.md** — Appends a row to the "Quick Tasks Completed" table (with a Status column when `--full`). Updates the "Last activity" line.
9. **Final commit** — Commits all artifacts: plan, summary, STATE.md, and (if `--full`) verification file.
...
flowchart TD
    ...
    TaskDir --> Planner[Spawn gsd-planner → N-PLAN.md]
...
- [`quick-task`](../../prompts/quick-task/) — Quick task workflow prompt
