...
Doctor runs checks across six categories:

- **Structure** — Missing summaries, UAT files, unchecked roadmap entries, missing `tasks/` directories, duplicate task IDs, premature slice completion
...
- **Runtime state** — Stale `auto.lock` (PID check), stranded lock directory, stale parallel sessions, orphaned completed-unit keys, stale hook state, activity log bloat, STATE.md staleness, gitignore drift, metrics ledger bloat
- **Global state** — Orphaned project state directories in `~/.gsd/projects/` whose git root no longer exists
- **Environment** — Provider/auth key availability for active work
