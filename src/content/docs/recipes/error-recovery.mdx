...
Doctor runs checks across seven domains:

- **Structural checks** — Missing summaries, UAT files, unchecked roadmap entries, missing `tasks/` directories, duplicate task IDs, premature slice completion, requirements without owners
...
- **Engine health** — DB constraints and projection drift between disk state and the GSD database
...
- **Stale runtime unit files** — Clears leftover `runtime/units/*.json` files for milestones that have already completed (have a SUMMARY on disk), preventing stale state from causing the wrong milestone to resume.
- **Index lock removal** — If `.git/index.lock` is older than 60 seconds (from a prior crash), it is removed automatically so git operations succeed.
...
Heal mode first applies all fixable repairs, then filters the remaining issues — all fixable errors and warnings — and dispatches them to the LLM as a structured list. The LLM receives the full doctor report with issue codes, unit IDs, and file paths, and is instructed to generate real artifacts from existing context (task summaries, plan files) rather than leaving placeholders when possible.
...
If both doctor and forensics can't automatically resolve the state, use the reset commands to restore a clean baseline, then resume.

**Stuck `execute-task`:**

1. Run `gsd undo-task T02` to reset the task's completion state (DB + markdown)
2. Resume auto-mode — it will re-execute the task from scratch
3. If the task keeps failing, run `gsd recover` to rebuild DB state from disk

**Stuck `complete-slice`:**

1. Run `gsd reset-slice S01` to reset the slice and all its tasks (DB + markdown)
...
- [`/gsd skip`](../../commands/skip/) — Prevent a unit from being dispatched by auto-mode
