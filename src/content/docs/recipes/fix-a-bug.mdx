...
:::tip
Want to front-load scope decisions for a slice before auto mode plans it? Run [`/gsd discuss`](../../commands/discuss/) after the roadmap exists — it runs a guided interview about slice behaviour, edge cases, and scope, and writes a `CONTEXT.md` that the planner reads during the next planning unit.
:::
...
- Reassesses the roadmap — confirms no further slices are needed (requires [`phases.reassess_after_slice`](../../commands/prefs/) preference)
...
| File | Purpose |
|------|---------|
| `.gsd/PROJECT.md` | Project description and milestone sequence — created or updated during the discuss phase |
| `.gsd/REQUIREMENTS.md` | Capability contract with stable `R###` IDs — written during the discuss phase |
| `.gsd/DECISIONS.md` | Architectural and pattern decisions — created by `gsd_decision_save` during discuss; appended to during task execution |
...
| `.gsd/milestones/M003/slices/S01/S01-ASSESSMENT.md` | UAT execution results written by the run-uat unit; overwritten with roadmap reassessment when `phases.reassess_after_slice` is enabled |
...
| `.gsd/REQUIREMENTS.md` | Capability contract read by research, planning, and validation phases |
...
| `.gsd/REQUIREMENTS.md` | Requirement status updated as capabilities are validated |
| `.gsd/DECISIONS.md` | Appended to when a task executor makes a notable architectural or pattern decision |
...
flowchart TD
    ...
    NewMilestone --> Context["Context + REQUIREMENTS.md written"]
    Context --> Auto["Auto-mode starts (or /gsd auto)"]
    Auto --> Research[Research codebase + slice]
    Research --> Plan[Plan milestone + slice tasks]
    Plan --> Execute[Execute tasks in fresh sessions]
    Execute --> Complete[Write slice summary + UAT]
    Complete --> RunUAT{UAT passes?}
    RunUAT -->|Yes| Reassess[Reassess roadmap]
    RunUAT -->|No| Fix[Fix and re-verify]
    Fix --> RunUAT
    Reassess --> Validate[Validate milestone]
    Validate --> Seal[Complete milestone + squash merge]
    Seal --> Done([Bug fixed ✓])
...
- [`/gsd discuss`](../../commands/discuss/) — guided slice interview after the roadmap exists
