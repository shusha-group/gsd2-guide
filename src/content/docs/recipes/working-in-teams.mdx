...
**What gets shared** (committed): `preferences.md`, `PROJECT.md`, `REQUIREMENTS.md`, `DECISIONS.md`, `QUEUE.md`, and all milestone plans, roadmaps, and summaries.

**What stays local** (gitignored): lock files, SQLite cache (including WAL sidecars), state cache, activity logs, runtime records, and worktrees.
...
After the merge completes, GSD asks whether to remove the worktree and milestone branch or keep them for continued parallel work. Choosing to remove cleans up the worktree directory and branch. Planning artifacts always remain in `milestones/` as a historical record and travel with the squash commit to `main`.

### Optional: Auto-create pull requests

If your team uses a code-review workflow before merging to main, set `auto_pr: true`. GSD will open a **draft** pull request after the milestone branch is pushed, targeting the configured `pr_target_branch` (defaults to `main`). This requires `auto_push: true` and the `gh` CLI installed and authenticated:
...
| `none` | No git isolation — commits land on the user's current branch | Hot-reload workflows where file watchers only see the project root |
...
5. **Clean up prompt** — After a successful merge, GSD asks whether to remove the worktree and milestone branch or keep them for continued parallel work. Removal runs `git worktree remove` and `git branch -D` to clean up the directory and branch. Planning artifacts remain in `.gsd/milestones/` as a historical record regardless.
...
Worktrees are cleaned up after merge (if the user confirms removal). Planning artifacts remain in `milestones/` as a historical record. Because planning artifacts are tracked in git (not gitignored), they travel with each milestone branch and merge cleanly to `main` as part of the squash commit.
...
flowchart TD
    ...
    MergeA --> CleanA{"Remove\nworktree?"}
    MergeB --> CleanB{"Remove\nworktree?"}
    CleanA -->|yes| Done([Both milestones merged ✓])
    CleanA -->|no| Done
    CleanB -->|yes| Done
    CleanB -->|no| Done
    ...
    style CleanA fill:#0d180d,stroke:#39ff14,color:#39ff14
    style CleanB fill:#0d180d,stroke:#39ff14,color:#39ff14
