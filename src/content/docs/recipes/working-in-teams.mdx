...
GSD creates a milestone with a unique ID and works in an isolated worktree on a single branch. All execution — research, planning, implementation — happens via sequential commits on `milestone/M001-eh88as`. No per-slice branches are created:
...
feat(M001/S01): plan
feat(M001/S01/T01): implement signup endpoint
feat(M001/S01/T02): implement session handling
...
When Alice's milestone finishes, GSD squash-merges her `milestone/M001-eh88as` branch to main. If `main` has diverged (e.g., another milestone merged first), GSD rebases before the squash to avoid conflicts. Pre-merge checks verify that:
...
flowchart TD
    ...
    CompleteA --> MergeA["Pre-merge check → rebase if needed\n→ squash to main"]
    CompleteB --> MergeB["Pre-merge check → rebase if needed\n→ squash to main"]
