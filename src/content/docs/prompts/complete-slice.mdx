...
The closer also performs knowledge capture: it appends significant architectural decisions to `.gsd/DECISIONS.md` and non-obvious lessons learned to `.gsd/KNOWLEDGE.md`, but only when the content would genuinely save future agents from repeating investigation. If execution produced evidence that a requirement changed status, the closer calls `gsd_save_decision` with `scope="requirement"` — it does **not** write `.gsd/REQUIREMENTS.md` directly, as the engine renders that file from the database. Similarly, if `.gsd/PROJECT.md` exists, the closer refreshes it as a full-document write using the `write` tool, not `edit`.
