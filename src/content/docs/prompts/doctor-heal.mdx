...
One safety invariant is enforced throughout: `doctor-heal` never queries `.gsd/gsd.db` directly via `sqlite3` or inline Node.js calls. It uses `gsd_milestone_status` instead, because direct database access bypasses the WAL connection owned by the engine and can corrupt in-flight writes.
...
|----------|-------------|----------|
| `doctorCommandSuffix` | Additional arguments or flags passed to the `/gsd doctor` invocation, used when mentally re-running the scan to confirm resolution | Yes |
