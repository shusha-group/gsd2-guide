...
For each check, the prompt selects the lightest evidence tool that honestly proves the check — shell commands, grep/ripgrep file searches, script invocations, file content reads, artifact structure verification, or browser/runtime/network inspection for live flows. When a check cannot be honestly automated, the best objective evidence is gathered and the check is marked `NEEDS-HUMAN`.
...
After running all checks, an overall verdict is computed: `PASS` if all required checks passed, `FAIL` if any check failed, or `PARTIAL` if some checks were skipped, inconclusive, or still require human judgment. The result is always written to `uatResultPath` in a fixed schema — YAML frontmatter with `sliceId`, `uatType`, `verdict`, and `date`; a checks table; an overall verdict with summary; and a notes section capturing any additional context, errors encountered, screenshots or logs gathered, and manual follow-up still required. This structured format lets the dispatcher mechanically read the verdict and decide whether to advance to the next slice or pause for human review.
