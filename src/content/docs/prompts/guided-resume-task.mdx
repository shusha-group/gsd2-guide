...
Skill preferences are also respected: if a `GSD Skill Preferences` block is present in system context, the executor uses it to decide which skills to load during the resumed session, without relaxing verification or artifact requirements.
