...
When meaningful architectural, library, or observability decisions are made during a task, the executor appends them to `.gsd/DECISIONS.md`. Non-obvious recurring gotchas or useful patterns go to `.gsd/KNOWLEDGE.md`. After completing its work, the executor writes a task summary (including a clear one-liner that becomes the commit message) and then calls `gsd_complete_task` with the milestone ID, slice ID, task ID, and a completion summary. This tool call is required — it marks the task complete, updates the database, and renders `PLAN.md` automatically. The executor never edits `PLAN.md` checkboxes directly and never runs git commands.
...
| `phaseAnchorSection` | Phase handoff anchor from the `plan-slice` phase, injected when available to carry forward key planning context into execution | Yes |
...
- [`/gsd next`](../../commands/next/) — dispatches a single `execute-task` unit in step mode, then pauses for the next decision
- [`/gsd`](../../commands/gsd/) — same dispatch engine as `/gsd next`; routes to `execute-task` when the next pending unit is a task
