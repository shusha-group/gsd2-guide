...
Rather than the elaborate depth-calibration and numbered steps of the auto-mode version, `guided-research-slice` uses five **Strategic Questions** to structure the agent's thinking: what to prove first, what existing patterns to reuse, what boundary contracts matter, what constraints the codebase imposes, and what failure modes should shape slice ordering. These questions ensure the output is decision-driving rather than just fact-collecting. The result is a `{sliceId}-RESEARCH.md` file written to the slice directory, consumed by `guided-plan-slice` or `plan-slice` in the next step.
...
flowchart TD
    ...
    GuidedResearchSlice --> ResearchArtifact["writes {sliceId}-RESEARCH.md"]
