...
description: "Capture triage prompt — classifies pending /capture notes against the current slice plan and roadmap, assigning each a disposition (quick-task, inject, defer, replan, or note) for user confirmation."
---
...
After classifying, the prompt presents its proposals to the user via `ask_user_questions` for any quick-task, inject, or replan items. Once confirmed, it updates `CAPTURES.md` for each capture — setting `Status: resolved`, adding the confirmed classification, resolution description, rationale, and a timestamp. It then summarizes what was triaged: how many captures, what classifications were assigned, and what actions are pending (e.g., "2 quick-tasks ready for execution, 1 deferred to S03"). Critically, the prompt does NOT execute any resolutions: quick-tasks are not run, injected tasks are not added. Execution happens separately, keeping the triage step focused and auditable.
...
|----------|-------------|----------|
| `pendingCaptures` | List of pending capture items from `CAPTURES.md` to be classified | Yes |
| `currentPlan` | Full content of the current slice plan, used to evaluate whether captures belong in the current slice | Yes |
| `roadmapContext` | Current roadmap content providing project-wide scope for defer vs. inject decisions | Yes |
