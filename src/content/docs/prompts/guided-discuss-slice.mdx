...
Before the first question round, the agent does a lightweight targeted investigation to ground its questions in reality: it scouts the codebase with `rg`, `find`, or `scout` to understand what already exists at the slice boundary, checks the roadmap context to understand what surrounds this slice, uses `resolve_library` / `get_library_docs` for unfamiliar libraries rather than web search, and uses `search_and_read` for one-shot topic research. The agent has a limited web search budget (typically 3–5 per turn), so it targets 2–3 searches during investigation and distributes any remaining ones across question rounds rather than clustering them. This pass identifies the 3–5 biggest behavioral unknowns — things where the user's answer will materially change what gets built — before any questions are asked.

The agent then runs 1–3 question rounds. When `{{structuredQuestionsAvailable}}` is `true`, it uses `ask_user_questions` for structured input (called exactly once per turn); otherwise it asks in plain text. Each round focuses on UX behavior, edge cases, scope boundaries, and the feel of the slice when done. After each round it decides whether to continue or wrap up; it does not ask a meta "ready to wrap up?" question after every round. When it does ask a wrap-up question, it offers two options: "Write the context file" (recommended when the slice is well understood) and "One more pass."

**Incremental persistence:** After every 2 question rounds, the agent silently saves a draft `{{sliceId}}-CONTEXT-DRAFT.md` using `gsd_summary_save` with `artifact_type: "CONTEXT-DRAFT"`. This protects confirmed work against session crashes without interrupting the interview flow.

**Non-bypassable gate:** The agent must not write the final context file until the user explicitly selects "Write the context file." Tool errors, non-responses, or the impression that the slice is well understood are not valid reasons to proceed — the gate is an instruction to wait, not an obstacle to work around.

The output is a `{{sliceId}}-CONTEXT.md` file with seven sections: Goal (one sentence), Why this Slice, In Scope (confirmed during the interview), Out of Scope (explicitly deferred or excluded), Constraints, Integration Points (what the slice consumes and produces), and Open Questions with current thinking. The agent writes this file by calling `gsd_summary_save` with the milestone ID, slice ID, artifact type `"CONTEXT"`, and the content — the tool writes to disk and persists to the database simultaneously. When writing is complete the agent says exactly `"{{sliceId}} context written."` and stops. This context file is read by `research-slice` and `plan-slice`, making it the primary mechanism for a user to inject non-obvious scope decisions into the planning process.
...
| `structuredQuestionsAvailable` | Whether `ask_user_questions` is available in this session; controls question format (structured vs plain text) | Yes |
