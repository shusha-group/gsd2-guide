...
When changes are needed, the reassessment agent rewrites only the unchecked slices in the roadmap — completed slices are never modified. It also considers deferred captures: user thoughts captured during execution and deferred to future consideration. These may warrant slice adjustments, reordering, or new scope additions. The assessment document it writes is brief and concrete: what changed, why, and what the coverage impact is. The pipeline then continues with the next slice in the updated roadmap.
