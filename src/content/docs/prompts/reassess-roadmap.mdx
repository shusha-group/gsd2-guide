...
The agent works through a structured set of questions before deciding: Did the slice retire the risk it was supposed to? Did new risks emerge that should change slice ordering? Are the boundary contracts still accurate given what was actually built? Are Threat Surface and Requirement Impact sections in remaining slice plans still valid? Did Operational Readiness findings reveal monitoring gaps that remaining slices should address? Should any Horizontal Checklist items be updated based on what was actually built?
...
When changes are needed, the reassessment agent rewrites only the unchecked slices in the roadmap — completed slices are never modified. Along with the slice content, it updates the boundary map for changed slices and revises the proof strategy if the risk landscape shifted. If a `.gsd/REQUIREMENTS.md` file exists, it also evaluates requirement coverage and updates ownership or status if scope changed. Deferred captures — user thoughts captured during execution and held for future consideration — are also reviewed and may warrant slice adjustments, reordering, or new scope additions.

All roadmap mutations go through the `gsd_reassess_roadmap` tool, which writes to the database and re-renders `ROADMAP.md` atomically. The assessment document it writes is brief and concrete: what changed, why, and what the coverage impact is. The pipeline then continues with the next slice in the updated roadmap.
