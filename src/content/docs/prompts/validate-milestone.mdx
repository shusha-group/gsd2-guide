...
The output is a structured validation report written to `{validationPath}`. The report file includes YAML frontmatter with `verdict` and `remediation_round` fields, followed by a success criteria checklist, a slice delivery audit table, cross-slice integration findings, requirement coverage check, verdict rationale, and (if needed) a remediation plan listing new slices to add to the roadmap. The validation pass is the last substantive review before the milestone branch is merged and the worktree is torn down.
