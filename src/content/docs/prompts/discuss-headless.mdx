...
The prompt executes a four-step flow: reflect on the specification to surface a concrete summary, investigate the codebase briefly (limited to 5-6 tool calls), make explicit decisions for any gaps or ambiguities, and then assess scope before writing artifacts. Any judgment calls made during that process are documented as assumptions in the context file, so a future agent or human reviewer can audit exactly what choices were made and why. The prompt explicitly prohibits asking the user questions — if information is missing from the specification, the prompt infers the most sensible default and documents it.
