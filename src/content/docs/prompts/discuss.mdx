...
flowchart TD
    ...
    Requirements --> ScopeAssess["scope assessment"]
    ScopeAssess --> RoadmapPreview["roadmap preview in chat"]
...
| `preparationContext` | Injected preparation context block assembled by the dispatcher — codebase snapshot, existing planning artifacts, and any prior discussion state the agent should be aware of before the investigation step | Yes |
| `structuredQuestionsAvailable` | Boolean flag indicating whether the `ask_user_questions` tool is available — controls whether question rounds, depth verification, and readiness gates use structured form UX or plain-text mode throughout the discussion | Yes |
