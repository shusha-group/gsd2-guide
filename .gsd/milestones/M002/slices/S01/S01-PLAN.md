...
- Prebuild stdout includes exclusion log lines (confirms filtering is observable, not silent)

## Tasks

- [x] **T01: Strip pi/agent content and restructure sidebar** `est:45m`
...
- [x] **T02: Author the "Developing with GSD" end-to-end walkthrough** `est:1.5h`
...
- **Prebuild exclusion reporting:** `scripts/prebuild.mjs` logs excluded files/directories to stdout during `npm run build`, making it visible when content is filtered out vs silently missing.
- **Build failure surface:** `npm run build` exit code and stderr surface broken links, missing references, and Astro compilation errors. Build output is the primary diagnostic for content integration issues.
- **Link checker:** `node scripts/check-links.mjs` enumerates all broken internal links with source file and target — the failure-path check for link integrity after content removal.
- **Sidebar verification:** The sidebar structure in `astro.config.mjs` is declarative and inspectable via `grep` or AST — no runtime state to observe, but malformed entries cause Astro build failures with clear error messages.
- **Content manifest:** `.generated-manifest.json` in `src/content/docs/` tracks exactly which files were generated by prebuild — diff-able to verify exclusions took effect.
