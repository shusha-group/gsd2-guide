...
- **Placeholder page rendering:** `find dist/gsd2-guide/placeholder -name "*.html" | wc -l` returns 3 (one per placeholder page). Absence means content pages weren't created or sidebar wiring is broken.
- **Mermaid rendering:** `grep -c "mermaid" dist/gsd2-guide/placeholder/diagrams/index.html` returns > 0. Zero means the Mermaid plugin isn't processing code fences.
- **Pagefind search index:** `ls dist/gsd2-guide/pagefind/*.js 2>/dev/null | wc -l` returns > 0. Empty means search indexing failed.
- **Sitemap generation:** `test -f dist/sitemap-index.xml` confirms R012. Missing means `site` config is wrong or @astrojs/sitemap is misconfigured.
- **Sidebar completeness:** Dev server shows "Placeholder" group with 3 items + autogenerated groups for S01 content subdirectories. Missing groups mean autogenerate directory paths don't match actual content paths.
- **Build page count:** `find dist -name "*.html" | wc -l` returns > 10 (placeholder + prebuild content). Low count indicates prebuild or sidebar routing issues.
