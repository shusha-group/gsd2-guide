...
- [ ] M003: Continuous Documentation Regeneration — LLM-powered pipeline that detects gsd-pi source changes between versions, maps them to affected doc pages, and regenerates only stale pages via Claude API. Handles new/removed commands automatically. Integrated into `npm run update`. S01 (Source Diff and Page Mapping) complete — 42 pages mapped to 485 source deps, diff detection and staleness resolver verified with 21 tests. S02 (LLM Page Regeneration) complete — `regeneratePage()` module with Claude API integration, quality prompt template with capture.mdx exemplar, 13 unit tests, quality verified against M02 originals (byte-identical for 3 pages). S03 (New/Removed Command Handling) complete — `manage-pages.mjs` module with detection, creation, and removal of command doc pages (7 exports, 31 tests, CLI entry point, mock-verified LLM integration). R040/R041/R044 validated. S04 remaining — pipeline integration.
