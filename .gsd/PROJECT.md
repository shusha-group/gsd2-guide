...
**M005/S05 complete.** `manage-pages.mjs` extended with 5 prompt lifecycle functions (`detectNewAndRemovedPrompts`, `addPromptSidebarEntry`, `removePromptSidebarEntry`, `createNewPromptPages`, `removePromptPages`). `update.mjs` is now a 10-step pipeline with a "manage prompts" step. `regenerate-page.mjs` dispatches prompt-page-aware regeneration via `buildPromptSystemPrompt()` + `execute-task.mdx` exemplar. `page-versions.json` stamped with 80 total pages (32 prompts/ + 48 existing). Stale detection confirmed end-to-end. 54 manage-pages tests pass, 10 update-pipeline tests pass. `npm run build` exits 0 at 104 pages.

**M005 complete.** All 5 slices done: S01 (prompts.json extraction), S02 (32 stub MDX pages + sidebar + source map), S03 (full page content with Mermaid + variable tables), S04 (16 command backlinks), S05 (pipeline integration). 32 prompt pages live, 10-step update pipeline fully prompt-aware.
