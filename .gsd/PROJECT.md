...
- [ ] M004: Claude Code–Powered Documentation Regeneration — Replace the API-based regeneration from M003 with `claude -p` invocation (full tool access, no API key). Verify all 43 page-source-map entries. Prove the full update cycle end-to-end on real stale pages. "Update gsd-guide" completes with zero intervention.
