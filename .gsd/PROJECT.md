...
**M006 complete.** "Solo Builder's Guide" sidebar group published with 9 MDX pages (index + 8 content sections). Covers why GSD 2 exists, daily workflow decisions (8-row decision table), brownfield onboarding, context engineering, cost management, failure recovery (8 failure scenarios), and building a sustainable rhythm. All 8 sections are 102–183 lines of substantive narrative prose with 86 total cross-references and 5 external citations (Addy Osmani, Esteban Torres, The New Stack, Shareuhack, Daniel Priestley). Hand-authored content excluded from the update pipeline (D068) — will not be regenerated. npm run build exits 0 at 113 pages; npm run check-links exits 0 at 12,288 links. R061–R072 all validated. 39 upstream stale gsd-pi pages pending a standalone `npm run update` run.
...
- [x] M006: Solo Builder's Applied Guide to GSD 2 — Eight-section practitioner guide for solo founders graduating from vibe coding platforms. Covers why GSD 2 exists, daily workflow decisions, brownfield onboarding, cost management, failure recovery, and building a sustainable rhythm. Published as a new "Solo Builder's Guide" sidebar group on GitHub Pages. **All 8 slices complete.** All 8 content sections live (102–183 lines each). npm run build exits 0 at 113 pages; npm run check-links exits 0 at 12,288 links. R061–R072 all validated (R051 remains active — semantic audit deferred). 39 upstream gsd-pi stale pages pending a standalone `npm run update` run.
