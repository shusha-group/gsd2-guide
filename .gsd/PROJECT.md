...
- [ ] M004: Claude Code–Powered Documentation Regeneration — Replace the API-based regeneration from M003 with `claude -p` invocation (full tool access, no API key). S01 complete: regeneration engine rewritten (SDK→subprocess), 20 tests pass, integration proof on capture.mdx, @anthropic-ai/sdk removed. S02 remaining: full pipeline end-to-end with real stale pages, commit/push/deploy cycle.
